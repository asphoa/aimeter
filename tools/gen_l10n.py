...
ROWS = [
    ...
    ('c.schema', 'Response shape not recognised', '回應格式無法辨識', 'Forme de réponse non reconnue', 'Antwortform nicht erkannt'),
    ('p.missing', 'Expected field missing', '缺少預期欄位', 'Champ attendu absent', 'Erwartetes Feld fehlt'),
    ('p.invalid', 'Field present but unusable', '欄位存在但無法使用', 'Champ présent mais inutilisable', 'Feld vorhanden, aber unbrauchbar'),
    ...
    ('x.partial', 'Quota search may be incomplete — only recent sessions were scanned', '配額搜尋可能不完整——只掃了近期 session', 'Recherche de quota peut-être incomplète — seules les sessions récentes ont été parcourues', 'Kontingentsuche möglicherweise unvollständig — nur aktuelle Sitzungen gescannt'),
    ('n.toolarge', 'Response too large to read', '回應太大無法讀取', 'Réponse trop volumineuse à lire', 'Antwort zu groß zum Lesen'),
    ...
    ('o.err.key', 'Could not read key: %@', '讀不到金鑰：%@', 'Impossible de lire la clé : %@', 'Schlüssel nicht lesbar: %@'),
    ('o.err.net', '%@: connection failed (%@)', '%@：連線失敗（%@）', '%@ : échec de connexion (%@)', '%@: Verbindung fehlgeschlagen (%@)'),
    ('o.err.http', '%@: HTTP %@', '%@：HTTP %@', '%@ : HTTP %@', '%@: HTTP %@'),
    ('o.unknown.remaining', 'Remaining quota unknown — limit period does not match usage', '剩餘配額未知——限額週期與用量不符', 'Quota restant inconnu — période de limite incompatible avec l’usage', 'Verbleibendes Kontingent unbekannt — Limitperiode passt nicht zur Nutzung'),
    ...
    ('d.localrule', 'Peak/off-peak pricing (local rule)', '峰谷計價（本機規則）', 'Tarification pointe/creux (règle locale)', 'Spitzen-/Nebenzeittarif (lokale Regel)'),
    ...
    ('l.ollama.reported', 'Ollama-reported', 'Ollama 回報', 'Signalé par Ollama', 'Von Ollama gemeldet'),
    ...
    ('m.timeout', 'Timed out', '逾時', 'Délai dépassé', 'Zeitüberschreitung'),
    ('m.retrying.early', 'Retrying before backoff ends', '退避結束前重試', 'Nouvelle tentative avant la fin du délai', 'Erneuter Versuch vor Ablauf der Wartezeit'),
    ('m.dropped', 'Dropped stale refresh result', '丟棄過期刷新結果', 'Résultat d’actualisation obsolète ignoré', 'Veraltetes Aktualisierungsergebnis verworfen'),
    ('a11y.expand', 'Expand card', '展開卡片', 'Développer la carte', 'Karte erweitern'),
    ('a11y.collapse', 'Collapse card', '收起卡片', 'Réduire la carte', 'Karte einklappen'),
    ('a11y.expanded', 'Expanded', '已展開', 'Développée', 'Erweitert'),
    ('a11y.collapsed', 'Collapsed', '已收起', 'Réduite', 'Eingeklappt'),
    ('a11y.ring', '%@ %@ %@', '%@ %@ %@', '%@ %@ %@', '%@ %@ %@'),
    ('s.range.clamped', 'Some values were adjusted to a valid range: %@', '部分數值已調整到有效範圍：%@', 'Certaines valeurs ont été ajustées à une plage valide : %@', 'Einige Werte wurden auf einen gültigen Bereich angepasst: %@'),
    ('s.agy.autoask', 'Antigravity can now auto-check — enable hourly polling?', 'Antigravity 現在可自動查詢——要開啟每小時輪詢嗎？', 'Antigravity peut maintenant vérifier automatiquement — activer le contrôle horaire ?', 'Antigravity kann jetzt automatisch prüfen — stündliche Abfrage aktivieren?'),
