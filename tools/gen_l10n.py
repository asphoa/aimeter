...
ROWS = [
    ...
    ('c.refresh.offer', 'Press “Check now” — this app runs claude for you, then reads the refreshed token; macOS will ask for your keychain password once', '按「立即檢查」——本程式會替你跑一次 claude，再把刷新後的權杖讀回來；macOS 會問一次鑰匙圈密碼', 'Appuyez sur « Vérifier maintenant » — cette app lance claude pour vous puis lit le jeton renouvelé ; macOS demandera une fois le mot de passe du trousseau', '„Jetzt prüfen“ drücken — diese App führt claude für Sie aus und liest dann das erneuerte Token; macOS fragt einmal nach dem Schlüsselbund-Passwort'),
    ...
    ('c.signedout.blank', 'Claude Code is signed out on this Mac (its stored credential holds no token) — run claude in a terminal and sign in', '這台 Mac 上的 Claude Code 已登出（存放的憑證內沒有權杖）——請在終端跑 claude 重新登入', 'Claude Code est déconnecté sur ce Mac (l’identifiant stocké ne contient aucun jeton) — lancez claude dans un terminal et reconnectez-vous', 'Claude Code ist auf diesem Mac abgemeldet (die gespeicherten Zugangsdaten enthalten kein Token) — claude im Terminal starten und erneut anmelden'),
