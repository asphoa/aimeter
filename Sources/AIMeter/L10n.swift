...
enum L {
    ...
    private static let table: [String: (String, String, String, String)] = [
        ...
        "c.refresh.offer": ("Press “Check now” — this app will run claude auth status for you and read the refreshed token", "按「立即檢查」——本程式會替你跑一次 claude auth status，再把刷新後的權杖讀回來", "Appuyez sur « Vérifier maintenant » — cette app lancera claude auth status pour vous et lira le jeton renouvelé", "„Jetzt prüfen“ drücken — diese App führt claude auth status für Sie aus und liest das erneuerte Token"),
