...
enum L {
    ...
    private static let table: [String: (String, String, String, String)] = [
        ...
        "c.refresh.offer": ("Press “Check now” — this app runs claude once for you (one tiny Haiku request, charged to your own window) and reads the refreshed token", "按「立即檢查」——本程式會替你跑一次 claude（一個極小的 Haiku 請求，算在你自己的額度裏），再把刷新後的權杖讀回來", "Appuyez sur « Vérifier maintenant » — cette app lance claude une fois pour vous (une minuscule requête Haiku, imputée à votre propre quota) et lit le jeton renouvelé", "„Jetzt prüfen“ drücken — diese App führt claude einmal für Sie aus (eine winzige Haiku-Anfrage, auf Ihr eigenes Kontingent) und liest das erneuerte Token"),
